defmodule ConsistentHashRing.Worker do
  @moduledoc false
  use GenServer
    ...
    cond do
      monitor_nodes? ->
        ...
        ring = Enum.reduce(nodes, ring, fn node, acc ->
          cond do
            ConsistentHashRing.Utils.ignore_node?(node, node_blacklist, node_whitelist) ->
              acc
            :else ->
              ConsistentHashRing.add_node(acc, node)
          end
        end)
      ...
      :else ->
        ...
        ring = ConsistentHashRing.add_nodes(ring, nodes)
        {:ok, {ring, [], []}}
    end
  end

  def handle_call(:list_nodes, _from, {ring, _, _} = state) do
    {:reply, ConsistentHashRing.nodes(ring), state}
  end
  def handle_call({:add_node, node}, _from, {ring, b, w}) do
    {:reply, :ok, {ConsistentHashRing.add_node(ring, node), b, w}}
  end
  def handle_call({:add_node, node, weight}, _from, {ring, b, w}) do
    {:reply, :ok, {ConsistentHashRing.add_node(ring, node, weight), b, w}}
  end
  def handle_call({:add_nodes, nodes}, _from, {ring, b, w}) do
    {:reply, :ok, {ConsistentHashRing.add_nodes(ring, nodes), b, w}}
  end
  def handle_call({:remove_node, node}, _from, {ring, b, w}) do
    {:reply, :ok, {ConsistentHashRing.remove_node(ring, node), b, w}}
  end
  def handle_call({:key_to_node, key}, _from, {ring, _, _} = state) do
    {:reply, ConsistentHashRing.key_to_node(ring, key), state}
  end
  ...
  def handle_info({:nodeup, node, _info}, {ring, b, w} = state) do
    if ConsistentHashRing.Utils.ignore_node?(node, b, w) do
      {:noreply, state}
    else
      {:noreply, {ConsistentHashRing.add_node(ring, node), b, w}}
    end
  end
  def handle_info({:nodedown, node, _info}, {ring, b, w}) do
    {:noreply, {ConsistentHashRing.remove_node(ring, node), b, w}}
  end
end
